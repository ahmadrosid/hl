...
fn read_input(path: &str) -> Vec<char> {
    return read_file(path).chars().collect::<Vec<char>>();
}
...
#[test]
fn test_groovy() {
    let input = read_input("src/test/testdata/input/Groovy.groovy.stub");
    let expected = read_file("src/test/testdata/output/Groovy.html.stub");
    let actual = groovy::render::render_html(input);
    assert_eq!(expected, actual);
}
...
fn test_ruby() {
    ...
    let expected = read_file("src/test/testdata/output/ruby.html.stub");
    let actual = ruby::render::render_html(input);
    assert_eq!(expected, actual);
}
